...
app.get('/WorkSchdule/:userID', (req,res) => {
 ...
 app.get('/Employee/:LoginTableID', (req,res) => {
    ...
    connection.query(`  SELECT * FROM Clinic_Main.EmployeeInfoWithLogin WHERE EmployeeLoginID='${LoginID}';
                        UPDATE Clinic_Main.Appointment SET StatusID=1 WHERE AppointmentDate<CURDATE() AND 
                        DoctorID=(SELECT EmployeeID FROM Clinic_Main.EmployeeInfoWithLogin WHERE EmployeeLoginID='${LoginID}');`,(err, results) => {
        if(err) {
            return res.send(err)
        }
        else {
